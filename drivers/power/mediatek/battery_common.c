...
static ssize_t present_cmd_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	int len = 0;
	char desc[32];
	//unsigned int present_init = 0;
	...
	if (sscanf(desc, "%d", &precent_init) == 1) {

		battery_log(BAT_LOG_CRTI, "[precent_init_write] present_init=%d\n", precent_init);
		BMT_status.UI_SOC = precent_init;
    BMT_status.SOC = precent_init;
    battery_meter_reset();
    wake_up_bat();
		return count;
	} /*else {*/
		battery_log(BAT_LOG_CRTI, "  bad argument, echo [enable] > present_init\n");
	/*}*/
...
static const struct file_operations present_cmd_proc_fops = {
	.open = proc_utilization_open,
	.read = seq_read,
	.write = present_cmd_write,
};
...
static int mt_batteryNotify_probe(struct platform_device *dev)
{
    ...
    proc_create("battery_precent", 0777, battery_dir, &present_cmd_proc_fops);
		battery_log(BAT_LOG_CRTI, "proc_create present_cmd_proc_fops\n");
