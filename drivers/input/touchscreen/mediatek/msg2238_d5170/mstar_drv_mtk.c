...
#define SAMPLE_FWVER_DIJIN_MINOR 7
...
//static CLASS_ATTR(gesenable, 0777, NULL, NULL);
//static CLASS_ATTR(gesenable, S_IWUSR, NULL, NULL);
...
//    const char *vcc_i2c_name = "vcc_i2c";
   ...
   tpd->reg = g_ReguVdd;
	...
	{	
		...
		if(pMajor==0x01){
		len=sprintf(tp_info,"TP1 IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:%x.%x,Sample FirmWare:%x.%x","MSG2238","HELITAI",0x26,pMajor,pMinor,SAMPLE_FWVER_HELITAI_MAJOR,SAMPLE_FWVER_HELITAI_MINOR);
		}else{
