...
*/
	...
	msleep(400);
		...
		is_update = false;
		printk("sym vend_id=%x end\n",vend_id);
	#endif



	//	fts_read_reg(client, FTS_REG_VENDOR_ID, &vend_id);
//	printk("sym vend_id=%x end\n",vend_id);
	#if 0
	/* Reset CTP */
...
/*i	int sensor_id;
	...
	msleep(5);
	if(sw_firmware == RGK_SAMPLE_DIJING_VERSION)
		break;
}
	sensor_id = get_module_id();
	module_name = tp_modules_name[sensor_id];
	f (sensor_id == 1 || sensor_id == 0)
	len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x", "FT5436", module_name, client->addr,sw_firmware,RGK_SAMPLE_DIJING_VERSION);
	else
	len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x", "FT5436", module_name, client->addr,sw_firmware,RGK_SAMPLE_YUSHUN_VERSION);
*/
	int len;
unsigned char tp_info[512];
	if (vend_id ==0x67)
	len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x", "FT5436", "dijing", client->addr,ver,RGK_SAMPLE_DIJING_VERSION);
	else if(vend_id == 0x5f)
	len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x", "FT5436", "yushun", client->addr,ver,RGK_SAMPLE_YUSHUN_VERSION);
	else
	len = sprintf(tp_info, "TP IC :%s,vendor:%x,TP I2C adr : 0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x", "FT5436", vend_id, client->addr,ver,RGK_SAMPLE_DIJING_VERSION);	
