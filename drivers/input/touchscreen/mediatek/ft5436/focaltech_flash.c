...
static unsigned char CTPM_DJ_FW[] = {
	#include "FT5436I_ID67_5130_V0x15_20151218_app.i"
};
static unsigned char CTPM_YS_FW[] = {
	#include "FT5436I_Ref_S80430_R_D5130_ID5F_V0x13_20151104_app.i"
};
...
void fts_get_upgrade_array(void)
{
	...
	printk("sym %s chip_id = %x\n", __func__, chip_id);
	//sym add for chip_id can't be read
	//if(ret < 0 || chip_id==0 || chip_id==0xff)
		chip_id = 0x12;
	//endif
...
int fts_5x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		auc_i2c_write_buf[1] = auc_i2c_write_buf[2] = auc_i2c_write_buf[3] = 0x00;
              fts_i2c_read(client, auc_i2c_write_buf, 4, reg_val, 2);           
printk("sym reg_val[0] =%x,reg_val[1] =%x,fts_updateinfo_curr.upgrade_id_1 =%x,fts_updateinfo_curr.upgrade_id_2=%x\n",reg_val[0] ,reg_val[1],fts_updateinfo_curr.upgrade_id_1,fts_updateinfo_curr.upgrade_id_2);
             if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1 
                             && reg_val[1] == fts_updateinfo_curr.upgrade_id_2)
           //   if (reg_val[0] == 0x79
                 //           && reg_val[1] == 0x11)     //xinan
              {
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	*/
	printk("sym %s fts_updateinfo_curr.CHIP_ID=%x\n",__func__,fts_updateinfo_curr.CHIP_ID);
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x64))
	{
		...
		pbt_buf = CTPM_FW;
		i_ret = fts_6336GU_ctpm_fw_upgrade(client, pbt_buf, sw_fw_len);
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n",__func__);
	}
	else if ((fts_updateinfo_curr.CHIP_ID==0x54))
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x58))
	{
		...
		pbt_buf = CTPM_FW;
		/*call the upgrade function*/
		i_ret = fts_5822_ctpm_fw_upgrade(client, pbt_buf, sw_fw_len);
		if (i_ret != 0) 
		{
			dev_err(&client->dev, "[FTS] upgrade failed. err=%d.\n", i_ret);
		} 
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x59))
	{
	    	if (fw_len < 8 || fw_len > 54*1024) 
		{
    		...
    		pbt_buf = CTPM_FW;
	   	 /*call the upgrade function*/
    		i_ret = fts_5x26_ctpm_fw_upgrade(client, pbt_buf, sw_fw_len);
    		if (i_ret != 0) 
		{
    			dev_err(&client->dev, "[FTS] upgrade failed. err=%d.\n", i_ret);
    		} 
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x86))
	{
		...
		i_ret =  fts_8606_ctpm_fw_upgrade(client, pbt_buf, sw_fw_len);
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x87))
	{
		...
		i_ret =  fts_8716_ctpm_fw_upgrade(client, pbt_buf, sw_fw_len);
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x0E))
	{
		...
		pbt_buf = CTPM_FW;
		i_ret = fts_3x07_ctpm_fw_upgrade(client, pbt_buf, sw_fw_len);
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n",__func__);
	}
	return i_ret;
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	static u8 uc_tp_fm_ver = 0;
	u8 count = 0;
	int i_ret;

	
	msleep(1);
	vend_id = fts_ctpm_update_project_setting(client);

     	printk("sym fts_ctpm_auto_upgrade vend_id = %x \n",vend_id);
	/*********xinan*********/
	...
	{
		CTPM_FW = CTPM_DJ_FW;
		sw_fw_len = sizeof(CTPM_DJ_FW);
	}
	else if(vend_id == 0x5f)
	{
		CTPM_FW = CTPM_YS_FW;
		sw_fw_len = sizeof(CTPM_YS_FW);
	}
	else
	{
		CTPM_FW = NULL;
	...
	while(uc_tp_fm_ver == 0 && count++ < 10){
		fts_read_reg(client, FTS_REG_FW_VER, &uc_tp_fm_ver);
		msleep(1);
		printk("sym uc_tp_fm_ver = %x,count=%x\n",uc_tp_fm_ver,count);
		}
	count = 0;
	
	uc_host_fm_ver = fts_ctpm_get_i_file_ver();
	printk("sym vend_id=%x,uc_tp_fm_ver=%x,uc_host_fm_ver=%x,fts_updateinfo_curr.CHIP_ID==%x,sw_fw_len=%x\n",vend_id,uc_tp_fm_ver,uc_host_fm_ver,fts_updateinfo_curr.CHIP_ID,sw_fw_len);

	if ((uc_tp_fm_ver < uc_host_fm_ver )  &&   CTPM_FW!=NULL  ||uc_tp_fm_ver > RGK_SAMPLE_DIJING_VERSION  || uc_tp_fm_ver <= 0 )
	{
		msleep(100);
		printk("sym if (uc_tp_fm_ver == FTS_REG_FW_VER ||	uc_tp_fm_ver < uc_host_fm_ver ) \n");
