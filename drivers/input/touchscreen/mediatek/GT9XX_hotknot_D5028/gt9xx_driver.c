...
CHIP_TYPE_T gtp_chip_type = CHIP_TYPE_GT9F;
u8 gtp_clk_buf[6];
...
/*#ifndef MT6589
	...
	printk("sym sensor id =0x%x\n",sensor_id);
	#ifdef RGK_SAMPLE_VERSION
	len=sprintf(tp_info,"TP IC:%s,Sensor ID:%x,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:0x%x,Sample FirmWare:0x%x",ic_g,sensor_id,module_name_g,0x5d,ic_fw_version_g,RGK_SAMPLE_VERSION);
	#else
	len=sprintf(tp_info,"TP IC:%s,Sensor ID:%x,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:0x%x",ic_g,sensor_id,module_name_g,0x5d,ic_fw_version_g);
	#endif
        ...
        if ((finger == 0x00) && (CHIP_TYPE_GT9F == gtp_chip_type))
        {
            ...
            switch(rqst_data[2]&0x0F)
            {
                ...
                case GTP_RQST_HOTKNOT_CODE:
                    ...
                    goto exit_work_func;
                case 0x07:
                    GTP_INFO("Request 14k gesture code.");
                    ret = gup_load_overlay_system(0xC800, 0x9000, 0x3800);
                    if(SUCCESS == ret)
                    {
                         ...
                         #endif
                        GTP_DEBUG("GTP has been working in overlay doze mode!");
                    }
                    goto exit_work_func;
					
                case 0x08:
                    GTP_INFO("Request 10k advanced code + 4k init code.");
                    //gup_load_overlay_system(0xC800, 0x4800, 0x3800);
                    gup_recovery_from_gesture(0);
                    goto exit_work_func;
                case 0x09:
                    GTP_INFO("Request 4k hotknot prox code.");
                    gup_load_overlay_system(0xF000, 0xC800, 0x1000);
                    goto exit_work_func;
                case 0x0A:
                    GTP_INFO("Request 4k init code.");
                    gup_load_overlay_system(0xF000, 0x7000, 0x1000);
                    goto exit_work_func;
                default:
                    break;
            }
        ...
        while (retry++ < 10)
        {
          ...
          //        force_reset_guitar();   
            ...
            {  
                ...
                //overlay need load 4k init code.
                gup_load_init_code();
...
static void tpd_resume(struct early_suspend *h)
{
    ...
    //mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);	
	ret = request_irq(touch_irq, (irq_handler_t) tpd_eint_interrupt_handler,
		gtp_eint_trigger_type, "TOUCH_PANEL-eint", NULL);
	enable_irq(touch_irq);
    mutex_unlock(&i2c_access);
#endif
