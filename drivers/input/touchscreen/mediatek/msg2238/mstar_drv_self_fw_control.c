...
#include "msg22xx_helitai_update_bin_v11.h" // for MSG22xx   //helitai
#include "msg22xx_junda_update_bin_v4.h"   //junda
...
static void _DrvFwCtrlUpdateFirmwareBySwIdDoWork(struct work_struct *pWork)
{
	...
	if(pMajor==0xc){
		len=sprintf(tp_info,"UTP IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:%x.%x,Sample FirmWare:%x.%x","MSG2238","JUNDA",0x26,pMajor,pMinor,SAMPLE_FWVER_JUNDA_MAJOR,SAMPLE_FWVER_JUNDA_MINOR);
	}else{
		len=sprintf(tp_info,"UTP IC:%s,TP MODULE:%s,TP I2C ADR:0x%x,SW FirmWare:%x.%x,Sample FirmWare:%x.%x","MSG2238","HELITAI",0x26,pMajor,pMinor,SAMPLE_FWVER_HELITAI_MAJOR,SAMPLE_FWVER_HELITAI_MINOR);
	}
