...
#if 1//rgk wanghanfeng add for *#*#9375#*#* start (2015-12-1)
...
static int rgk_tp_info_show(struct seq_file *m, void *v)
{
	seq_printf(m, "%s\n", tp_proc_buffer);	
	return 0;
}

static int rgk_tp_info_open(struct inode *inode, struct file *file)
{
	return single_open(file, rgk_tp_info_show, NULL);
}

static const struct file_operations rgk_tp_info_proc_fops = {	
	.owner      = THIS_MODULE,
	.open       = rgk_tp_info_open,
	.read       = seq_read,
	.llseek     = seq_lseek,
...
{
	struct proc_dir_entry *tp_info_entry = proc_create("rgk_tpInfo", 0444, NULL, &rgk_tp_info_proc_fops);
	if (tp_info_entry == NULL)
		printk("create /proc/rgk_tpInfo fail\n");
	
	return 0;

}
#endif //rgk wanghanfeng add for *#*#9375#*#* end (2015-12-1)
