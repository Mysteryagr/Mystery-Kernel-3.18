...
//#define LOG_1 xlog_printk("GC2365,MIPI 2LANE\n")
/****************************   Modify end    *******************************************/

#define printk(format, args...)    xlog_printk(PFX, "[%s] " format, __FUNCTION__, ##args)
...
//    xlog_printk("Exit! shutter =%d, framelength =%d\n", shutter,imgsensor.frame_length);
...
{
//    xlog_printk("le:0x%x, se:0x%x, gain:0x%x\n",le,se,gain);
...
//                xlog_printk("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
//            xlog_printk("Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
//            xlog_printk("Warning: current_fps %d fps is not support, so use cap's setting: %d fps!\n",imgsensor.current_fps,imgsensor_info.cap.max_framerate/10);
...
//    xlog_printk("scenario_id = %d\n", scenario_id);
    ...
    switch (scenario_id) {
        ...
        case MSDK_SCENARIO_ID_SLIM_VIDEO:
            ...
            break;
        default:
//            xlog_printk("Error ScenarioId setting");
            preview(image_window, sensor_config_data);
...
{
//    xlog_printk("enable = %d, framerate = %d \n", enable, framerate);
    spin_lock(&imgsensor_drv_lock);
...
//    xlog_printk("scenario_id = %d, framerate = %d\n", scenario_id, framerate);
...
//                    xlog_printk("Warning: current_fps %d fps is not support, so use cap's setting: %d fps!\n",framerate,imgsensor_info.cap.max_framerate/10);
        ...
        default:  //coding with  preview scenario by default
            ...
            spin_unlock(&imgsensor_drv_lock);
            set_dummy();
//            xlog_printk("error scenario_id = %d, we use preview scenario \n", scenario_id);
            break;
    }
    return ERROR_NONE;
}
...
//            xlog_printk("current fps :%d\n", (UINT32)*feature_data);
            spin_lock(&imgsensor_drv_lock);
            imgsensor.current_fps = *feature_data;
            spin_unlock(&imgsensor_drv_lock);
            break;
        case SENSOR_FEATURE_SET_HDR:
//            xlog_printk("ihdr enable :%d\n", (BOOL)*feature_data);
            spin_lock(&imgsensor_drv_lock);
            imgsensor.ihdr_en = (BOOL)*feature_data;
            spin_unlock(&imgsensor_drv_lock);
            break;
        case SENSOR_FEATURE_GET_CROP_INFO:
//            xlog_printk("SENSOR_FEATURE_GET_CROP_INFO scenarioId:%d\n", (UINT32)*feature_data);
...
//            xlog_printk("SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",(UINT16)*feature_data,(UINT16)*(feature_data+1),(UINT16)*(feature_data+2));
