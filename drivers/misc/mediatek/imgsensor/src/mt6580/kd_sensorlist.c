...
#if 1//rgk wanghanfeng add for *#*#9375#*#* start (2015-11-30)
#include <linux/seq_file.h>
static char rgk_camera_back[32]= "NoBackCamera";
static char rgk_camera_front[32] = "NoFrontCamera";

static int Main_clock =0;
static int Sub_clock =0;

static int rgk_camera_info_show(struct seq_file *m, void *v)
{
	seq_printf(m, "BackCamera:%s,FrontCamera:%s\n", rgk_camera_back,rgk_camera_front);	
	return 0;
}
static int rgk_camera_Mclk_info_show(struct seq_file *m, void *v)
{
	seq_printf(m, "Main_Mclk = %d,Sub_Mclk = %d\n",Main_clock,Sub_clock);	
	return 0;
}


static int rgk_camera_info_open(struct inode *inode, struct file *file)
{
	return single_open(file, rgk_camera_info_show, NULL);
}
static int rgk_camera_Mclk_info_open(struct inode *inode, struct file *file)
{
	return single_open(file, rgk_camera_Mclk_info_show, NULL);
}


static const struct file_operations rgk_camera_info_proc_fops = {	
	.owner      = THIS_MODULE,
	.open       = rgk_camera_info_open,
	.read       = seq_read,
	.llseek     = seq_lseek,
	.release    = single_release,
};

static const struct file_operations rgk_camera_Mclk_info_proc_fops = {	
	.owner      = THIS_MODULE,
	.open       = rgk_camera_Mclk_info_open,
	.read       = seq_read,
	.llseek     = seq_lseek,
	.release    = single_release,
};
...
int rgk_creat_proc_camera_info(void)
{
	struct proc_dir_entry *camera_info_entry = proc_create("rgk_cameraInfo", 0444, NULL, &rgk_camera_info_proc_fops);
...
int rgk_creat_proc_camera_Mclk_info(void)
{
	struct proc_dir_entry *camera_info_entry = proc_create("rgk_camera_Mclk_Info", 0444, NULL, &rgk_camera_Mclk_info_proc_fops);
	if (camera_info_entry == NULL)
		printk("create /proc/camera_info_entry fail\n");
			...
			if (DUAL_CAMERA_MAIN_SENSOR == g_invokeSocketIdx[i]) {
				ret =
				    ...
				    SensorGetInfo((MSDK_SCENARIO_ID_ENUM) (*pScenarioId[0]),
				       ...
				       //Add by xiaofei.yan
     					Main_clock = SensorInfo[0].SensorClockFreq;
					 PK_ERR("main_clock = %d\n",Main_clock);
				...
				//Add by xiaofei.yan
				Sub_clock = SensorInfo[1].SensorClockFreq;
				PK_ERR("sub_clock = %d\n",Sub_clock);
			}
...
/* FIX-ME: linux-3.10 procfs API changed */
	...
	atomic_set(&g_CamHWOpening, 0);
#if 1//rgk wanghanfeng add for *#*#9375#*#* start (2015-11-30)
    rgk_creat_proc_camera_info();
#endif//rgk wanghanfeng add for *#*#9375#*#* end (2015-11-30)
    rgk_creat_proc_camera_Mclk_info();
