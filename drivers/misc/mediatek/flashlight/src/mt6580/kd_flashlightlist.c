...
static struct platform_device *flashPltFmDev = NULL;
...
struct platform_device* get_flashlight_platform_device(void){
		return flashPltFmDev;
}
...
#define FLASHLIGHT_MAJOR 242
	...
	if(!flash_create_attr(&(get_platform_driver()->driver))){
	//			PK_DBG("create flash enable attr fail\n");
	}
...
static int flashlight_remove(struct platform_device *dev)
{
	...
	flash_delete_attr(&(get_platform_driver()->driver));
	logI("[flashlight_probe] Done ~");
	return 0;
}
...
#ifdef CONFIG_OF
static const struct of_device_id camera_flashlight_of_match[] = {
	{.compatible = "mediatek,kd_camera_flashlight"},
	{},
};
#endif
...
static struct platform_driver flashlight_platform_driver = {
	...
	.driver = {
		   ...
		   .owner = THIS_MODULE,
		   	#ifdef CONFIG_OF
				.of_match_table = camera_flashlight_of_match,
				#endif
		   },
};
...
struct platform_driver * get_platform_driver(void){
	return &flashlight_platform_driver;	
}
